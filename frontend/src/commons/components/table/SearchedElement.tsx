...
const SearchedElement = ({ searchFilter, text }: SearchElementProps) => {
	...
	return (
		<>
			<span>{text.slice(0, searchFilterIndex)}</span>
			<span className="searched">{text.slice(searchFilterIndex, searchFilterIndex + searchFilter.length)}</span>
			<span>{text.slice(searchFilterIndex + searchFilter.length)}</span>
		</>
	);
};
