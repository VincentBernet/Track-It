...
import {
  getArtistsName,
  type playlistType,
} from "../../../features/pages/easy-modification/EasyModificationUtils";
import {
  addToLikedTracks,
  checkIfTrackIsSaved,
...
const TrackCardList = ({
  ...
  const [tracksData, setTracksData] = useState<tracksEnrichedDataType | null>(
    null
  );
  const [tracks, setTracks] = useState<tracksEnrichedDataItemType[] | null>(
    null
  );
  const [successFetchingTracks, setSuccessFetchingTracks] = useState<
    boolean | null
  >(null);
  const [errorFetchingTracks, setErrorFetchingTracks] = useState<string | null>(
    null
  );
  const [title, setTitle] = useState<string>("Liked Songs");

  // TableOptions for sorting tracks and display / hiding columns state
  const [tableOptions, setTableOptions] = useState<tableOptionsType>(
    initialSortByOptionValue
  );
  ...
  const fetchDataIsSaved = async (firstData: tracksDataType) => {
    try {
      const likedTracksIds = firstData.items.map(
        (track: tracksDataItemType) => track.track.id
      );

      let fullResponse: string[] = [];
      // Spotify API only allows to check 50 tracks at a time
      for (let i = 0; i < likedTracksIds.length; i += 50) {
        const { data } = await checkIfTrackIsSaved(
          likedTracksIds.slice(i, i + 50)
        );
        fullResponse = [...fullResponse, ...data];
      }

      const dataItemsWithLiked: tracksEnrichedDataItemType[] =
        firstData.items.map((currTrack: tracksDataItemType, i) => {
          return {
            added_at: currTrack.added_at,
            track: {
              ...currTrack.track,
              isSaved: !!fullResponse[i],
            },
          };
        });
      const dataWithLiked = {
  ...
  const sortedTracks = useMemo(() => {
    ...
    if (tableOptions.artist.isAscending !== undefined) {
      if (tableOptions.artist.isAscending === true) {
        return sortedTracks.sort((a, b) =>
          getArtistsName(a.track.artists).localeCompare(
            getArtistsName(b.track.artists)
          )
        );
      }
      return sortedTracks.sort((a, b) =>
    ...
    if (tableOptions.duration.isAscending !== undefined) {
      if (tableOptions.duration.isAscending === true) {
        return sortedTracks.sort(
          (a, b) => a.track.duration_ms - b.track.duration_ms
        );
      }
      return sortedTracks.sort(
        (a, b) => b.track.duration_ms - a.track.duration_ms
      );
    }
  ...
  const handleSort = (selectedColumnName: columnNames) => {
    ...
    newTableOption[selectedColumnName].isAscending =
      !tableOptions[selectedColumnName].isAscending;
    setTableOptions(newTableOption);
    return;
  };
  ...
  const handleDisplay = (selectedColumnName: columnNames) => {
    ...
    temporaryTableOptions[selectedColumnName].isDisplayed =
      !tableOptions[selectedColumnName].isDisplayed;
