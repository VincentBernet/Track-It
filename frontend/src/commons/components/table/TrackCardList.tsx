...
import { type Playlist, getArtistsName } from "../../../features/pages/easy-modification/EasyModificationUtils";
import {
	addToLikedTracks,
	checkIfTrackIsSaved,
...
import { type ColumnNames, type TableOptions, initialSortByOptionValue } from "./Utils";
...
const TrackCardList = ({ selectedTracksUris, visiblePlaylist, handleSelectedTracks }: TrackCardListProps) => {
	...
	const [tableOptions, setTableOptions] = useState<TableOptions>(initialSortByOptionValue);
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: TODO => test if bug fix it
	useEffect(() => {
	...
	const handleSort = (selectedColumnName: ColumnNames) => {
		const newTableOption: TableOptions = {
			date_added: {
	...
	const handleDisplay = (selectedColumnName: ColumnNames) => {
		const temporaryTableOptions: TableOptions = { ...tableOptions };
